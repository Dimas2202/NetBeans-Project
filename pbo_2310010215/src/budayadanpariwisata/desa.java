...
public class desa {
    ...
    public String CEK_NAMADESA=null;
    ...
    public void simpandesa01(String id, String nm){
        try {
            String sqlsimpan="insert into desa(id, namadesa) VALUES"
                    + " ('"+id+"', '"+nm+"')";
            String sqlcari="select*from desa where id='"+id+"'";
     ...
     public void simpandesa02(String id, String nm) {
    try {
        String sqlsimpan = "INSERT INTO desa(id, namadesa) VALUES (?, ?)";
        String sqlcari = "SELECT * FROM desa WHERE id=?";
        ...
        if (data.next()) {
            ...
            this.duplikasi = true;
            this.CEK_NAMADESA = data.getString("namadesa");
        } else {
            this.duplikasi = false;
            this.CEK_NAMADESA = null;
     ...
     public void ubahDesa(String id, String nm){
        try {
            String sqlubah="UPDATE desa SET namadesa=?WHERE id=?";        
      ...
      public void hapusDesa(String id){
        try {
            String sqlhapus="delete from desa where id=?";        
      ...
      public void tampilDataDesa(JTable komponentabel, String SQL){
        try {
            ...
            modeltabel.addColumn("ID Desa");
            modeltabel.addColumn("Nama Desa");
            while(data.next()){
